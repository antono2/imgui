...
my $file_in  = $ARGV[0] // (-f 'cimgui.v' ? 'cimgui.v' : 'src/imgui.v');
my $file_out = $ARGV[1] // 'src/imgui.v';
my $module_name = 'imgui';
my $prefix = 'ImGui';
my $snake_prefix = 'im_gui_';
...
sub process_enum_block {
  ...
  foreach my $line (split /\n/, $body, -1) {
    if ($line =~ /^(\s*)(\w+)(\s*=\s*)([^\n]+?)(\s*)$/) {
      ...
      if (($name eq 'PopupFlags_' && $clean eq 'none') || $clean =~ /^(?:mouse_button_default_|fitting_policy_default_|pressed_on_default_|round_corners_default_|mod_none|named_begin)$/) {
        push @out, $ind . '//' . $clean . $eq . $expr;
      } elsif ($clean =~ /^(?:supported_by_is_key_pressed|cond_default_)$/) {
        push @out, $ind . '/* ' . $clean . $eq . $expr . ' */';
      } else {
        push @out, $ind . $clean . $eq . $expr;
      }
...
$content =~ s/^module\s+main\s*$/module $module_name/mg;
$content =~ s/\bmain\.//g;
$content =~ s/\bMain\.//g;
