...
# They were not translated from C
my @c_types_already_defined_by_imgui = (
  "va_list",           "ImVec4",
  "ImRect",            "ImWchar",
  "ImGuiDockRequest",  "ImGuiDockNodeSettings",
  "ImGuiInputEvent",   "ImGuiSelectionExternalStorage",
  "ImGuiSizeCallback", "ImGuiIDStackTool",
  "ImGuiTabBar"
);
...
# If they are still there, they point to an enum without value assignment
# "ImGuiDataType_.data_type_count" -> "int(ImGuiDataType_.data_type_count)"

# Change C.Time_t -> C.time_t. It's coming from time.h
#        C.Tm     -> C.tm

# Remove type defintion for C.ImPlotInputMap, as it was added as static string

# Remove im_plot for functions
final_cleanup();
...
# Get fn ( (params) ) (\w+), so it can be appended to @func_param_content and processed again
        ...
        # Note these don't contain a param names and are only types in a function defintion
...
# Ignore fn ( .. struct member types. These are found in @func_param_content
  ...
  # but it can not be an alias, like C.name
...
sub basic_cleanup {
  ...
  $content =~ s/             # Replace
          ...
          (\bImPlot_?)(?=\w+) # The thing to remove
          //gx;
...
sub append_static_strings {
  $header =~
    qr/^\/\/based\son\simplot.h\sfile\sversion\s([[:print:]])\sfrom\simplot/m;
  my $version_str = $1;
...
# InputMap or ImPlotInputMap is not translated from cimplot.h for some reason. Add it here
...
#  float ZoomRate;
#};
  my $struct_input_map = q|pub struct InputMap {
...
|;
  ...
  #print "\#####PRE STRUCT SCOPE CONTENT\n";
  #use Data::Dumper;
  #print Dumper( \%struct_scope_content );
  refresh_struct_scope_content();

  #print "\#####POST STRUCT SCOPE CONTENT\n";
  #print Dumper( \%struct_scope_content );
  refresh_needs_c_prefix_array();
  ...
  foreach my $type_needs_c_prefix (@needs_c_prefix_array) {
    ...
    # Make sure this C.type is not already defined in src/imgui.v
    # This does not need another type definition
    my $is_already_defined = 0;
    foreach my $already_defined (@c_types_already_defined_by_imgui) {
    ...
    if ( not $is_already_defined ) {
      $static_string .= "pub type C.$type_needs_c_prefix = voidptr\n";
    }
  }
...
sub remove_enum_member_prefix {
  ...
  if (  $is_in_another_enum
    ...
    and $member_name_or_scope_content =~ /im_plot_/ )
  {
    # Single line and value contains "."
    $member_name_or_scope_content =~ s/\b(.+)\.(im_plot_)(.+)/$1.$3/g;
    $enum_name = $1;
...
#"processed HoveredFlagsPrivate_.allowed_mask_for_is_item_hovered: $val_complete_clean\n";
  ...
  # If there still are enum members with alias values, run again
  ...
  for my $e_name ( keys %enum_scope_content ) {
    if (
      ...
      [\s<\d\|]+                 # Any base value
      (?!int\()                  # Not an int cast
      ([a-z_]+)                  # Mandatory alias. Match means run again
      \s*$                       # End of line
      /gmx
      )
    {
...
sub final_cleanup {
  ...
  # Remove type defintion for C.ImPlotInputMap, as it was added as static string
  $content =~ s/pub\stype\sC\.ImPlotInputMap\s=\svoidptr//;
  $content =~ s/C\.ImPlotInputMap\b/InputMap/g;

  # Remove im_plot for functions
  $content =~ s/\bfn\sim_plot_/fn /g;
}
