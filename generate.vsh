...
fn cd(dir string) {
  chdir(dir) or {
    panic('Could not change working directory to ${dir}')
  }
}
...
fn mcp(src string, dst string) {
  cp(src, dst) or {
    panic('Could not copy ${src} to ${dst}. ${err}')
  }
}
...
fn copy_by_ext(src string, dst string, ext []string) {
  ...
  for entry in entries {
    ...
    if !is_dir(fpath)
     && arrays.binary_search(ext, file_ext(entry)) or { continue } > -1 {
      mcp(fpath, '${dst}/${entry}')
    }
  }
}
...
cd('${wd}/include')
write_file('${wd}/include/c2v.toml', '[project]\nadditional_flags = "${dflags}"') or { panic('Could not write ${wd}/include/c2v.toml') }
sh('v translate cimgui.h')
sh('v translate cimplot.h')

println(" --- Move implot&gui.v")
mv('${wd}/include/cimgui.v', '${wd}/src/imgui.v', overwrite: true) or { panic('Could not move ${wd}/include/cimgui.v to ${wd}/src') }
mv('${wd}/include/cimplot.v', '${wd}/src/implot.v', overwrite: true) or { panic('Could not move ${wd}/include/cimplot.v to ${wd}/src') }

println(" --- Cleanup src/implot&gui.v")
cd(wd)
sh('${wd}/cleanup_imgui.perl')
sh('${wd}/cleanup_implot.perl')
// TODO: Uncomment once fmt doesn't exit with error, because of deprecated inline /* comment */
//sh('v fmt -w src/imgui.v')
//sh('v fmt -w src/implot.v')

println('Move ImPlot to its own submodule')
mv('${wd}/src/implot.v', '${wd}/implot/implot.v', overwrite: true) or { panic('Could not move ${wd}/src/implot.v to ${wd}/implot/') }

println("--- Build vimgui")
sh('cmake ${dflags} ${cflags} .')
sh('make')

println('GENERATED')
println('${wd}/src/imgui.v')
